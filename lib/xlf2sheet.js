...
exports.convert = async function convert(argv) {

    let xlfAsJson = await retrieveJsonFromxlf(argv);

    await writeWorkBook(argv, xlfAsJson);

}

async function retrieveJsonFromxlf(argv) {
...
async function writeWorkBook(argv, jsonData) {
    ...
    //Write the sheet
    xlsx.writeFile(wb, argv.out) //File name will be ?? and what about local
}
